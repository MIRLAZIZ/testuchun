<template>
    ...
    {{ newsData }}
                    ...
                    <div class="flex justify-between">
                        <div>
                            <p class="font-normal text-lg text-[#5D5D5F]"> <button
                                    @click="$router.push('/')">Asosiy</button> / <button>{{ newsData?.title }}</button>
                            </p>
                        </div>
                        ...
                        <div class="flex gap-6">
                            <div class="flex gap-2">
                                ...
                                <p class="font-normal text-base text-[#5D5D5F] wrapper_bot">{{
                                    newsData?.date?.substring(0, 10) }}</p>
                            </div>
            ...
            <UiNewsCarousel :items="store.news?.data?.slice(0, 6)"  :title="store.dataTranslate['home\.latest_news']" class="my-[104px]" />
        </div>
    </div>
...
onMounted(() => {
    store.getNewsOne(route.params.slug)
        .then((res) => {
